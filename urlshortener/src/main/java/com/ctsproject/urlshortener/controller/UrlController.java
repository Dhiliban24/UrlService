...
public class UrlController {
	...
	@GetMapping("/urlNuserById/{id}")
	public UserUrlMapper findUserAndUrlByUserId(@PathVariable("id") Long id){
		return urlShorteningService.fetchUserAndUrlById(id);
	}
	
	
	@PostMapping("/updateuserid")
	public Urls updateUserId(@RequestBody Urls url,Long id) {
		return urlShorteningService.updateUserId(url,id);
	}
 
	@PutMapping("/updateCount/{url}")
	
	public Urls updateCount(@PathVariable("url") String shortUrl){
		Urls url= urlShorteningService.findUrlsByShortUrl(shortUrl);
		LocalDate today=LocalDate.now();
		url.setLastAccessDate(today);
		return urlShorteningService.updateUrlCount(url);
	}
